...
# Scale x0 for not showing relative errors in graph
yerr = [i*0 for i in yerr]    


# plt.plot(x,Logistic_Paper(x,K,r,co))
plt.ylim(0,100)
plt.errorbar(x,y,yerr=yerr,capsize=2,fmt='m^',mfc='none',label=f"Generalized Growth Model\np={round(p,4)}, r={round(r,4)}")
