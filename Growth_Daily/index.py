...
(K,r,co),cov = optim.curve_fit(Logistic_Paper,x,y,bounds=[0,np.inf],p0=[1,0.01,90])
...
for trial in range(1,200):
    ...
    (a,b,c),cov = (optim.curve_fit(Logistic_Paper,x,y_dash,bounds=[0,np.inf],p0=[1,0.01,90]))
    K_ar.append(a)
    r_ar.append(b)
    co_ar.append(c)
...
# Scale x0 for showing relative errors in graph
yerr = [i*0 for i in yerr]    

plt.ylim(0,100)
# plt.plot(x,Logistic_Paper(x,K,r,co))
plt.errorbar(x,y,yerr=yerr,capsize=2,fmt='b^',mfc='none',label=f"Logistic Model\nC0={round(co,4)}, K={round(K,4)}, r={round(r,4)}")
